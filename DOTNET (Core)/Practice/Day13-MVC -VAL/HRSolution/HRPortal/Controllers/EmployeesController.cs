...
//Partial View
 ...
 static string pmsg=null;
    [HttpPost]
    public IActionResult ForgotP(string username, string Password)
    {
        bool z;
        Employee e=new Employee();
        e.username=username;
        EmployeeManager emgr=new EmployeeManager();
        bool y=emgr.ValidatePass(e);

        if(y==true)
        {
            if( username!="" && Password!="")
            {
            EmployeeManager emgr1=new EmployeeManager();
            Employee e1=new Employee();
            e1.username=username;
            e1.Password=Password;
            z=emgr1.UpdatePass(e1);
            pmsg="Password Updated Successfully !";
            }
        }
        else
        {
            pmsg="Incorrect user details !";
