...
public class Day20_2 {
	...
	public static void main(String[] args) 
	{
		...
		//get the numbers greater than 20
		System.out.println("Getting the data whose value is >20");
		listOfNumbers.stream()
		.flatMap(list -> list.stream())
		.filter(n -> n>20)
		.forEach(System.out::println);
		
		System.out.println("Get the numbers >25 and multiply them with 2 and display list");
		listOfNumbers.stream()
		.flatMap(list -> list.stream())
		.filter(n -> n>25)
		.map(n -> n*2)
		...
		System.out.println("Get the numbers >25 and map to int and then display sum");
		
		System.out.println(listOfNumbers.stream()
		.flatMap(list -> list.stream())
		.filter(n -> n>25)
		.mapToInt(n -> n*1)
		.sum());
